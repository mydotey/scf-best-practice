...
public class LargeApp {
	...
	public LargeApp() {
		// OwnManagerComponent both users config and defines how to config properties
		// itself
		_ownManagerComponent = new OwnManagerComponent();

		// OuterManagerComponent only uses config, let the component's user to define
		// how to config
		...
		// LabeledPropertyComponent only uses config, let the component's user to define
		// how to config
		// each user can define different config way
		...
		// large app's config
		PropertiesFileConfigurationSourceConfig sourceConfig2 = SimpleConfigurationSources
				.newPropertiesFileSourceConfigBuilder().setName("properties-file").setFileName("app.properties")
				.build();
		ConfigurationSource source2 = SimpleConfigurationSources.newPropertiesFileSource(sourceConfig2);
